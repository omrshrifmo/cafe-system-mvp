...
function modeMiddleware(req, res, next) {
  ...
  // If in ONBOARDING mode, block all API requests except for the setup routes
  if (currentMode === MODES.ONBOARDING) {
    if (req.path.startsWith('/api/') && !req.path.startsWith('/api/setup') && req.path !== '/api/build-info') {
